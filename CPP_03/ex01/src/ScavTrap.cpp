...
void	ScavTrap::attack(std::string const &name)
{
	if (getHitP() == 0)
	{
		std::cout << "ScavTrap " << getName() << " is in bad shape, it might be good to give up!" << std::endl;
		return ;
	}
	if (getEnergyP() > 0)
	{
		...
		std::cout << "ScavTrap " << getName() << " is exausted, he can't do anything anymore!" << std::endl;
	return ;
}
